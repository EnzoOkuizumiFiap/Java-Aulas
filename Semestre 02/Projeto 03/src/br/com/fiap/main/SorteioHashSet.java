...
public class SorteioHashSet {
    public static void main(String[] args) {
        ...
        Random random = new Random(); //Random -> Aleatório, é para gerar um número aleatório

        do {
            while (numerosSorteados.size() < 6) {
                int numero = random.nextInt(1, 61); //nextInt() faz o sorteio.
                numerosSorteados.add(numero);
            }
            ArrayList<Integer> resultadoDoSorteio = new ArrayList<>(numerosSorteados);
            Collections.sort(resultadoDoSorteio);

            JOptionPane.showMessageDialog(null, "Resultado do Sorteio do Tigrinho\n" + resultadoDoSorteio, "Resulado", JOptionPane.WARNING_MESSAGE);

            numerosSorteados.clear(); //Limpando o numerosSorteados
